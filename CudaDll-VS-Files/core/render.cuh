...
struct Float4FloatPair {
    float4 first; // Represents float4
    float second; // Represents float
    float third;
};

//<int type = Type::MRPNN, int BatchSize = 4, int StepNum = 1024>
template<int type = Type::MRPNN, int BatchSize = 4 >
__device__ Float4FloatPair NNPredict(float3 ori, float3 dir, float3 lightDir, float3 lightColor = { 1, 1, 1 }, float alpha = 1, float g = 0, int StepNum = 1024) {
    ...
    if (type!= -1) {
        ...
        if (!aint) {
            returner.first = make_float4(SkyBox(dir), -1);
            return returner;
        }
        ...
        */
        ...
        returner.first = make_float4(active > 0 ? predict * lightColor * scatter_rate : SkyBox(dir), active > 0 ? distance(ori, pos) : -1);
        return returner;
    }
    else {
        bool unfinish = true;
        ...
        // predict
        {
            ...
            if (!aint) {
                ...
                returner.first = make_float4(ref, hitDis);
                //returner.first = make_float4(ref, hitDis);
                return returner;
            }
            ...
            //write the transmittence to the surface, remove 

            //returner.first = make_float4(ref + lerp(res * lightColor, skybox(dir), transmittance), hitDis);
            returner.first = make_float4(ref + res * lightColor, hitDis);
            returner.second = transmittance;
