...
using namespace std;
extern Timer timerA;  // Declare global timer
extern Timer timerB;  // Declare global timer
...
public:
	...
	VolumeRender(string path, int targetResolution);
	...
	void Render(float3* taeget, Histogram* histo_buffer, unsigned int* target2, int2 size, float3 ori, float3 up, float3 right, float3 lightDir, float3 lightColor = { 1,1,1 }, float alpha = 1, int multiScatter = 1, float g = 0, int randseed = 0, RenderType rt = RenderType::PT, int toneType = 2, bool denoise = false,cudaSurfaceObject_t surface = 0, cudaSurfaceObject_t infoSurface = 0);
	void saveTimings();

	void set_Step_Num(int stepNum);

};


void debug_something();